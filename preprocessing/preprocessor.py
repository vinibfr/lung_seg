...
class MakeDataSet:
    def __init__(self, LIDC_Patients_list, IMAGE_DIR, MASK_DIR,CLEAN_DIR_IMAGE,CLEAN_DIR_MASK,META_DIR, mask_threshold, padding, confidence_level=0.5):
        ...
        self.col = ['patient_id','nodule_no','slice_no','original_image','mask_image','malignancy','is_cancer','is_clean']
        self.meta = pd.DataFrame(index=[],columns=self.col)
    ...
    def save_meta(self,meta_list):
        new_row_dict = dict(zip(self.col, meta_list))
        self.meta = self.meta._append(new_row_dict, ignore_index=True)

...
def train_acc(train_loader, model, criterion, optimizer, accumulation_steps):
    avg_meters = {'loss': AverageMeter(),
                  'iou': AverageMeter(),
                  'dice': AverageMeter()}
    ...
    accumulated_gradients = {param: torch.zeros_like(param.data) for param in model.parameters()}
    total_iterations = 0
    ...
    for input, target in train_loader:
        ...
        for param in model.parameters():
            accumulated_gradients[param] += param.grad
        ...
        if total_iterations % accumulation_steps == 0:
            # Update the model parameters using the accumulated gradients
            for param in model.parameters():
                param.grad = accumulated_gradients[param] / accumulation_steps
...
def main():
    ...
    meta = pd.read_csv('../preprocessing/data/meta_sep.csv')                    
    ############################################################################
    ...
    best_dice = 0
    trigger = 0
    accumulation_steps = 4  # You can adjust this number based on your requirements
    ...
    for epoch in range(config['epochs']):
        ...
        train_log = train_acc(train_loader, model, criterion, optimizer, accumulation_steps)

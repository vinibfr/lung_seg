    ...
    for input_folder, output_folder in zip(input_folders, output_folders):
        ...
        for file_name in file_list:
            ...
            if file_name.endswith('.npy'):
                ...
                if npy_array.dtype == np.bool_:
                   # Convert boolean values to integers (0 or 255)
                   normalized_array = npy_array.astype(np.uint8) * 255
                else:
                    # Normalize other data types to the range [0, 255]
                    normalized_array = ((npy_array - npy_array.min()) / (npy_array.max() - npy_array.min()) * 255).astype(np.uint8)

                rgb_array = np.stack((normalized_array,) * 3, axis=-1)
